...
def init_cheese():
    action_template = [
            ...
            "walk to the cheese",  # 4
            "walk to the microwave",  # 5

            "grab the cheese",  # 6

            "put the cheese in the microwave",  # 7

            'open the microwave',  # 8
            'close the microwave',  # 9
        ]
    obs2text = obs2text_cheese
    return action_template, obs2text


def obs2text_cheese(obs, action_template):
    text = ""
    ...
    if in_kitchen:
        ...
        if not see_cheese:
            object_text += "The cheese is in the microwave. "
        else:
            object_text += "You notice cheese and microwave. "

        if hold_cheese:
            object_text += "Currently, you have grabbed the cheese in hand. "
            ...
            if close_to_cheese and not close_to_microwave:
                object_text += "Currently, you are not grabbing anything in hand. The cheese is close to you. "
                action_list = [0, 2, 3, 5, 6]
            elif close_to_microwave and not close_to_cheese:
                object_text += "Currently, you are not grabbing anything in hand. The microwave is close to you. "
                action_list = [0, 2, 3, 4, 8, 9]
            elif not close_to_cheese and not close_to_microwave:
                object_text += "Currently, you are not grabbing anything in hand. The cheese and the microwave are not close to you. "
                action_list = [0, 2, 3, 4, 5]
            else:
        ...
        if see_cheese and is_microwave_open:
            object_text += "The microwave is opened. "
        elif see_cheese and not is_microwave_open:
        ...
        if hold_cheese:
            object_text += "and notice nothing useful. Currently, you have grabbed the cheese in hand. "
        else:
    ...
    target_template = "In order to heat up the cheese in the microwave, "
